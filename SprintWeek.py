...
# Written by Glen May, Mark Hannem & Jacob Eagles (Group 22A3)
# Written between February 25th 2022 and March 6th 2022
# This program was written for Sprint Week and contains various functions described below.

# Common modules between functions.
from datetime import datetime
from datetime import timedelta
import matplotlib.pyplot as plt # matplotlib must be installed to run this software
...
# Calculates travel claims for the Newfoundland Chocolate Company based on company policy.
# Please see the supplied Technical Manual for further information on claim reimbursement.
def travelClaim():
    # Validation subsets
    ...
    while True:
        ...
        while True:
            ...
            elif blank(EmpNum) == False:
                break
        while True:
            EmpFirst = input("Enter the employee's first name: ").title()
            if set(EmpFirst).issubset(allowed_char) == False:
                print("Employee's first name contains invalid characters - please re-enter.")
            elif blank(EmpFirst) == False:
                break
        while True:
            EmpLast = input("Enter the employee's last name: ").title()
            if set(EmpLast).issubset(allowed_char) == False:
                print("Employee's last name contains invalid characters - please re-enter.")
            elif blank(EmpLast) == False:
                break
        while True:
            City = input("Enter the destination city: ")
            if set(City).issubset(allowed_char) == False:
                print("Destination city contains invalid characters - please re-enter.")
            elif blank(City) == False:
                break
        while True:
            Country = input("Enter the destination country: ")
            if set(Country).issubset(allowed_char) == False:
                print("Destination country contains invalid characters - please re-enter.")
            elif blank(Country) == False:
                TripLoc = City + "," + " " + Country
                break
        while True:
            try:
                TripStart = input("Enter the start date of the business trip (YYYY-MM-DD): ")
                TripStart = datetime.strptime(TripStart, "%Y-%m-%d")
            except:
        ...
        while True:
            ...
            if OwnCar != "O" and OwnCar != "R":
                print("Input must be \"R\" for employee rental or \"O\" for employee owned - please re-enter.")
            elif blank(OwnCar) == False:
                break
        if OwnCar == "O":
            while True:
                TotKilos = input("Enter the total number of kilometers traveled (must be < 2000km): ")
                if blank(TotKilos) == False:
                    try:
                        TotKilos = int(TotKilos)
                    except:
                        print("Total kilometers must be a whole number - please re-enter")
                if TotKilos > 2000:
                    print("Total kilometers traveled cannot exceed 2000 - please re-enter.")
                elif TotKilos <= 0:
                    print("Total kilometers must be greater than 0.")
                else:
                    break
        else:
            TotKilos = 0
        while True:
            ...
            elif blank(ClaimType) == False:
                break

        # Calculate travel claim and format data for user display
        ...
        if TripStart.month == 12 and 15 < TripStart.day < 22:
        ...
        print("Trip Start Date:                         {}".format(TripStart))
        print("Trip End Date:                           {}".format(TripEnd))
        print("Number of Days Spent on Trip:            {}".format(NumDays))
        print("Did Employee Rent or Use Own Vehicle?:   {}".format(OwnCarDsp))
        if OwnCar == "O":
            print("Total Kilometers Travelled:              {} km".format(TotKilos))
        print("Employee Travel Claim Type:              {}".format(ClaimTypeDsp))
        ...
        print()
        Continue = input("Type 'Q' to quit, or any other key to input another claim.: ").upper()
        if Continue == "Q":
            break
...
def strDate():
    ...
    CUR_DATE = datetime.now()  # Can be changed to fiscal year-end date if program ran outside intended usage.
    ...
    # Calculate employee age, tenure and bonus. Format display values.
...
# Function that graphs total sales for the Newfoundland Chocolate Company
def Graph():
    # Function for Graph
...
# Main program. Requires user input to call on above-defined functions.
main()
while True:
    ...
    if userChoice == "":
        ...
        print("Entry not recognized as an option.")
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    print("October must have a valid number entry - please re-enter.")
                else:
                    Claims.append(Oct)
                    break
            while True:
